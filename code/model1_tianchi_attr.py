...
def test_acc_fe(model):
    ...
    for i in range(test_feature.shape[0]):
        output_label = kNNClassify(y_pred[i, :], test_embeddings_per_class, test_id_per_class, 1)
...
def train_acc_fe(model):
    ...
    y_pred = model.predict(train_for_vali_feature)
    if type(y_pred) == type(list()):
        y_pred = np.stack(y_pred, axis=1)
        y_pred = np.squeeze(y_pred)
    ...
    for i in range(train_for_vali_feature.shape[0]):
        output_label = kNNClassify(y_pred[i, :], train_embedding_per_class, train_id_per_class, 1)
        out_pred[i] = output_label
...
class acc_callback_fe(keras.callbacks.Callback):
    def on_epoch_end(self, batch, logs=None):
        ...
        print_in_file('train acc :{}\ttest acc:{}'.format(train_accuracy, test_accuracy),
                      '../result/model1_tianchi_attr.txt')
...
if __name__ == '__main__':
    ...
    model = dem(train_feature.shape[1], train_embeddings_per_img.shape[1], norm_rate=1e-5)

    loss_decay = keras.callbacks.ReduceLROnPlateau(monitor='loss', patience=5, verbose=1, factor=0.5, min_lr=1e-7)

    checkpoint = keras.callbacks.ModelCheckpoint(filepath='../zsl_model/model1_{}.h5'.format(embed_type), monitor='loss', verbose=1,
                                                 mode='auto',
                                                 save_best_only=True,
    ...
    model.fit(x=train_feature[shuffle_index[:-1000]], y=train_embeddings_per_img[shuffle_index[:-1000]],
