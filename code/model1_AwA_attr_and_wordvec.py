...
def test_acc_fe(model):
    global x_test, test_id, test_label, test_word_per_class
    ...
    for i in range(x_test.shape[0]):
        output_label = kNNClassify(y_pred[i, :], test_word_per_class, test_id, 1)
...
class acc_callback_fe(keras.callbacks.Callback):
    def on_epoch_end(self, batch, logs=None):
        ...
        print_in_file(
            ...
            '../result/model1_AwA_{}.txt'.format(embed_type))
...
if __name__ == '__main__':
    ...
    train_word = np.hstack((att, train_word))
    ...
    test_word_per_class = np.hstack((att_pro, test_word_per_class))
    ...
    model = dem(x.shape[1], train_word.shape[1], norm_rate=1e-3)
    ...
    model.summary()
    model.fit(x=x, y=train_word, batch_size=1024, epochs=100000, callbacks=callback_lists,
              shuffle=True)
