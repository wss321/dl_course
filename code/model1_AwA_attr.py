...
def test_acc_fe(model):
    ...
    for i in range(x_test.shape[0]):
        output_label = kNNClassify(y_pred[i, :], att_pro, test_id, 1)
...
if __name__ == '__main__':
    ...
    model = dem(x.shape[1], att.shape[1], norm_rate=1e-4)
    ...
    model.summary()
    model.fit(x=x, y=att, batch_size=1024, epochs=100000, callbacks=callback_lists,
              shuffle=True)
